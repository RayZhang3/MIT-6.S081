# 总览
每个Lab对应不同的Branch
1. Xv6 and Unix utilities: 实现几个用户态程序、unix实用工具，包括pingpong, 用多线程实现并行化的素数筛
2. system calls: 添加syscall系统追踪，sysinfo系统调用
3. page tables:==理解进程页表、内核页表概念==，为每个进程维护独立的内核态页表，加速copyin的过程。
4. traps: 中断、中断处理机制(trap, trampoline, 函数调用、现场保存、页表/特权切换、调用栈、栈指针、返回指针)
5. lazy page allocation: lazy-allocation，内存懒分配，调用sbrk()时不立即分配内存，只作记录，在访问到这一部分内存也并触发缺页异常时才进行实际的物理内存分配。
6. copy-on-Write Fork: 写时复制的fork, 在进程fork后与父进程共享进程页，仅当其中一方尝试对内存页进行修改时才对内存也进行复制。
7. multithreading: 实现简单的用户态线程库，尝试使用线程为程序提速。
8. locks: 重新设计代码，降低锁竞争，提高多核系统的性能。包括并发的内存分配、物理缓存页管理。
9. file system: 为xv6的文件系统添加大文件以及符号链接的支持。
10. 1mmap: unix调用mmap的简单版本，支持将文件映射到一片用户虚拟内存区域内，并支持将对其的修改写回磁盘。
11. networking: 熟悉系统驱动与外围设备的交互、内存映射寄存器与DMA数据传输，实现与E1000网卡交互的核心方法：transmit和recv.
